...
def readValue(x1,y1,x2,y2,filename):
	screenshot = ImageGrab.grab(bbox =(x1,y1,x2,y2), include_layered_windows=False)	#take screenshot of correct area
	img = screenshot.convert('L')			#make the image grayscale to make ocr easier
	# resize image
	w,h = img.size							#get original size
	dimention = (w*3, h*3)
	resized = img.resize(dimention)			#tripple image size to get optimal height for ocr (30 pixels)
